...
<span class="hljs-tag">&lt;<span class="hljs-name">script</span> <span class="hljs-attr">src</span>=<span class="hljs-string">"https://cdn.jsdelivr.net/npm/scandit-sdk"</span>&gt;</span><span class="undefined"></span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
			...
			<p>A specific version of the library (example for 1.4.7) can also be included with:</p>
			<pre><code class="lang-html"><span class="hljs-tag">&lt;<span class="hljs-name">script</span> <span class="hljs-attr">src</span>=<span class="hljs-string">"https://unpkg.com/scandit-sdk@1.4.7"</span>&gt;</span><span class="undefined"></span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
<span class="hljs-comment">&lt;!-- or --&gt;</span>
<span class="hljs-tag">&lt;<span class="hljs-name">script</span> <span class="hljs-attr">src</span>=<span class="hljs-string">"https://cdn.jsdelivr.net/npm/scandit-sdk@1.4.7"</span>&gt;</span><span class="undefined"></span><span class="hljs-tag">&lt;/<span class="hljs-name">script</span>&gt;</span>
</code></pre>
  ...
  preloadCameras: <span class="hljs-built_in">boolean</span> = <span class="hljs-literal">false</span>
			...
			<p>The following configuration options lets you decide if you would like to eagerly load the external Scandit Engine library and/or the available cameras as soon as this function is called (by default they are not). The library load could take some time due to the download size and performance needed to prepare the external library, while the camera load could also take some time and ask the user for camera access permissions if needed. The required loads can be made later on via <code>ScanditSDK.loadEngineLibrary()</code> and <code>ScanditSDK.CameraAccess.getCameras()</code>, or are made later on when needed, if never done before. Please note that the <code>preloadEngineLibrary</code> option or the <code>ScanditSDK.loadEngineLibrary()</code> method only downloads the library and puts it in a local cache if available (then loads it and verifies the license key). When the library is then required and used by a <code>BarcodePicker</code> instance the external Scandit Engine library needs to be parsed and executed again and thus still requires some time to be ready. To make the process faster it&#39;s recommended, if possible, to instead prepare in advance library and cameras via a hidden <code>BarcodePicker</code> instead, as shown later.</p>
    ...
    <span class="hljs-keyword">return</span> <span class="hljs-built_in">string</span> + <span class="hljs-string">`<span class="hljs-subst">${Barcode.Symbology.toHumanizedName(barcode.symbology)}</span>: <span class="hljs-subst">${barcode.data}</span>\n`</span>;
			...
			<ul>
				...
				<li><p>Library performance</p>
					...
					<ul>
						...
						<li>If possible, create a (hidden and paused) <code>BarcodePicker</code> in the background and simply resume scanning by switching its visibility to &quot;show&quot; when needed, instead of creating it on the fly to provide instant scanning capabilities. You can also set it up to not access the camera immediately if needed and lazily do that later when needed.</li>
			...
			<ul>
				...
				<li><p>Why do I randomly get &quot;Out of bounds memory access&quot; or &quot;None: abort(6)&quot; errors?</p>
					<p>This is caused by a iOS 11.2.2 / 11.2.5 / 11.2.6 <em>Safari</em> browser, due to <em>WebKit</em> memory access problems introduced by this specific iOS versions by Apple, as indicated in the <a href="#important-notes">Important Notes</a> section. This is unfortunately out of our control, but a fix for it is expected to be released by the responsible browser/OS teams in iOS 11.3 (currently in beta).</p>
				</li>
			</ul>
				...
				<ul class="tsd-descriptions">
					<li class="tsd-description">
						...
						<div class="tsd-comment tsd-typography">
							...
							<p>Please note that preloading the Scandit Engine library only downloads the library and puts it in a local cache
								if available (then loads it and verifies the license key). When the library is then required and used by a
								<a href="classes/barcodepicker.html">BarcodePicker</a> instance the external Scandit Engine library needs to be parsed and executed again and thus still
								requires some time to be ready. To make the process faster it&#39;s recommended, if possible, to instead prepare
							in advance library and cameras via a hidden <a href="classes/barcodepicker.html">BarcodePicker</a> instead.</p>
						</div>
